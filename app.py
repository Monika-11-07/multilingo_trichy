...
async def generate_telegram_qr_code(current_user: Dict = Depends(verify_token)):
    ...
    try:
        ...
        token = result[0]
        bot_username = "Multilingotrain_bot"  # Replace with your actual bot username
...
async def train_status_page():
    ...
    try:
        with open("static/train_status.html", "r") as f:
            return HTMLResponse(content=f.read())
    except FileNotFoundError:
        return HTMLResponse("""
        <html>
...
async def get_train_status(train_number: str, current_user: Dict = Depends(verify_token)):
    ...
    try:
        ...
        url = "http://127.0.0.1:5001/api/train/status"
        querystring = {"trainNo": train_number, "startDay": "1"}
        headers = {
            "x-rapidapi-key": "675afd7d96msh6bb097aca8b80edp167ae0jsn6fdcef34d94a",
            "x- -host": "indian-railway-irctc.p.rapidapi.com",
