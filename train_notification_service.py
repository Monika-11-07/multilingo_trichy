...
class TrainNotificationService:
    def __init__(self, 
                 ...
                 telegram_token: str = "8327968816:AAHAV5BCDfG3IERUr5IPne_1wJyCJTQTiBY",
                 mock_api_url: str = "http://localhost:5001/api",
    ...
    def translate_with_gemini(self, text: str, target_language: str) -> str:
        ...
        try:
            ...
            client = genai.Client(api_key="AIzaSyBwUWCGI0z54hn0oylWRcFhtlVnTg58uZ0")
    ...
    def generate_audio_file(self, text: str, language: str = "en") -> Optional[str]:
        ...
        try:
            ...
            # Validate language support with gTTS
            ...
            logger.info(f"Generating audio in {lang_code} for text length: {len(prepared_text)}")
            
            # Generate audio with prepared text
            tts = gTTS(text=prepared_text, lang=lang_code, slow=False)
            tts.save(file_path)
            
            # Verify file was created
            if os.path.exists(file_path) and os.path.getsize(file_path) > 0:
                logger.info(f"Audio file created successfully: {filename}")
                return file_path
            else:
                logger.error(f"Audio file was not created or is empty: {filename}")
                return None
