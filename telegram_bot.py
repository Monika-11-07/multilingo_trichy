...
TOKEN = '8327968816:AAHAV5BCDfG3IERUr5IPne_1wJyCJTQTiBY'
...
def get_user_token_by_chat_id(chat_id: str) -> str:
    ...
    try:
        response = requests.post('http://localhost:8000/api/telegram/get-token', 
...
def get_user_preferences(chat_id: str):
    ...
    try:
        ...
        response = requests.get('http://localhost:8000/api/user/preferences',
...
def update_user_audio_notifications(chat_id: str, enable: bool):
    ...
    try:
        ...
        response = requests.post('http://localhost:8000/api/user/preferences',
                               json=prefs,
                               headers={'Authorization': f'Bearer {token}'})
...
def get_user_subscriptions(chat_id: str):
    ...
    try:
        ...
        response = requests.get('http://localhost:8000/api/train-subscription/list',
                              headers={'Authorization': f'Bearer {token}'})
        if response.status_code == 200:
            data = response.json()
...
def subscribe_to_train(chat_id: str, train_number: str):
    ...
    try:
        ...
        response = requests.post('http://localhost:8000/api/train-subscription/subscribe',
...
def unsubscribe_from_train(chat_id: str, train_number: str):
    ...
    try:
        ...
        response = requests.delete(f'http://localhost:8000/api/train-subscription/unsubscribe/{train_number}',
                                 headers={'Authorization': f'Bearer {token}'})
        return response.status_code == 200
    except Exception as e:
...
def get_train_status_from_api(train_number: str):
    ...
    try:
        response = requests.get(f'http://localhost:5001/api/train/status?train_id=train_{train_number}', timeout=10)
        if response.status_code == 200:
            return response.json()
        return None
    except Exception as e:
...
def process_message(chat_id, text, username=None):
    ...
    try:
        ...
        # Handle verification tokens (8 characters, alphanumeric, uppercase)
            ...
            try:
                response = requests.post('http://localhost:8000/api/telegram/verify-token', 
